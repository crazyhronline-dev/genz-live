...
export async function saveArticleAction(formData: FormData): Promise<void> {
  ...
  // Server-side publishing permission check
...
/** 4. UPDATE ARTICLE STATUS (Approve / Review / Archive / Publish) */
export async function updateArticleStatusAction(articleId: string, newStatus: ArticleStatus): Promise<void> {
  const user = await getCurrentUser();
  if (!user) redirect('/admin/login');

  if ((newStatus === 'PUBLISHED' || newStatus === 'SCHEDULED') && !hasPermission(user.role, ['ADMIN', 'EDITOR'])) {
