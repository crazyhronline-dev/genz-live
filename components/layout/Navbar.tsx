...
import {
  Search, Radio, Menu, X, Sparkles, Bookmark,
  TrendingUp, Globe, Cpu, Bot, Briefcase, Film, Trophy, Palette, Flame,
} from 'lucide-react';
...
export default function Navbar({
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-slate-950/80 backdrop-blur-xl border-b border-white/10">
        ...
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:inline text-slate-300">
              YouTube: <strong className="text-purple-400">GenZ Live ({SITE_CONFIG.youtube.handle})</strong>
            </span>
          </div>
          <div className="flex items-center gap-4 text-slate-400">
            <span className="hidden md:inline">{currentDate}</span>
            <div className="flex items-center gap-3">
              <a href={SITE_CONFIG.youtube.url} target="_blank" rel="noreferrer" title="YouTube">
                <YoutubeIcon className="w-3.5 h-3.5 text-red-500 hover:text-red-400 transition-colors" />
              </a>
              <a href={SITE_CONFIG.social.instagram} target="_blank" rel="noreferrer" title="Instagram">
                <InstagramIcon className="w-3.5 h-3.5 text-pink-500 hover:text-pink-400 transition-colors" />
              </a>
              <a href={SITE_CONFIG.social.facebook} target="_blank" rel="noreferrer" title="Facebook">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        <a href="/" className="flex items-center group">
          <img
            ...
            onError={(e) => { (e.target as HTMLImageElement).src = '/brand/08_Website_Logo_300x100.png'; }}
          />
        </a>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setSearchOpen(!searchOpen)} className="md:hidden p-2 rounded-full bg-slate-900 border border-white/10 text-slate-300">
            <Search className="w-4 h-4" />
          </button>
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 rounded-full bg-slate-900 border border-white/10 text-slate-300">
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-slate-950 border-b border-white/10 px-4 py-4">
