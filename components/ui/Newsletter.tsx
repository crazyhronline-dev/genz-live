      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="relative rounded-3xl overflow-hidden p-8 md:p-12 border border-purple-500/30 bg-gradient-to-r from-purple-950/80 via-slate-900 to-cyan-950/80 shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl mx-auto text-center space-y-6">
            ...
            {subscribed ? (
              <div className="bg-emerald-950/80 border border-emerald-500/40 p-4 rounded-2xl flex items-center justify-center gap-3 text-emerald-300 font-bold text-sm">
                <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                <span>You&apos;re subscribed! Welcome to the {SITE_CONFIG.name} community.</span>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-3 bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 text-white font-extrabold text-xs rounded-full shadow-lg shadow-purple-900/40 transition-all transform hover:scale-105 flex items-center justify-center gap-2 disabled:opacity-60"
                >
                  {loading ? 'Subscribing…' : <><span>Subscribe Now</span><Send className="w-3.5 h-3.5" /></>}
                </button>
              </form>
            )}
            <p className="text-[11px] text-slate-500">No spam. Unsubscribe anytime with one click.</p>
          </div>
        </div>
      </div>
    </section>
